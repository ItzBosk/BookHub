"""Config views module for the marketplace application."""
...
"""
Description for past_researches.
"""
# lista ricerche
@login_required
...
def new(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            query = form.save(commit=False)  # non salvo subito nel db perché non saprei chi hacreato il prodotto
            query.user = request.user
            query.save()
            return redirect(results, query_id=query.id)  # research salvata, redirect a risultatiquery
