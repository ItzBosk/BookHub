"""Module for item views in the marketplace application."""
...
def items(request):
    ...
    if genre_id:
        items_list = items.filter(genre_id=genre_id)

    # ricerca per lingua, mostra solo libri di quella lingua
    if language_id:
        items_list = items.filter(language_id=language_id)

    # filtro le query con titolo o descrizione oggetto
    if query:
        # Q permette di cercare in campi multipli
        items_list = items.filter(Q(title__icontains=query) | Q(description__icontains=query) |
